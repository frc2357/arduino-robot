...
private:
    ...
    static const int SERIAL_PREAMBLE_LENGTH; // Length of preamble used for serial comms
    static const int DATA_LENGTH;            // Length of all fields (Total number of fields)
...
public:
    ...
    bool buildSerialTransmission(uint8_t *transmission, uint8_t len);
    bool readSerialMessage(uint8_t *message, uint8_t len);
